...
Xcode translates the source code, in the language Swift for our app, into binary code, and gathers all the Swift files created by us. Xcode then linkes the code and these assets, or files. Xcode then performs code signing, which the project owner must initiate by choosing their Apple ID as their team in the Signing and Capabilities settings of the CS50 Project. Camino Claro, the IOS app, code can be found in a Library folder within the user's MacOS. If testing the app, the user must use Xcode. The process to compile the app can also be known as a "build". To build the app, the project owner must open the project in Xcode first. They must paste the github repository link, provided in the submission, to open the repository. To intiate a build, the user must open the project in Xcode and must choose the destination of the app (which must be a simulator device). To choose the destination of the app, the user must click next to the app icon on the top middle panel. The user must then choose a simulator, preferably the iPhone 17 simulator. If this simulator is not seen, the user must click Manage Run Destinations, toggle to simulators and change the setting of "Show run destination" to "always". Once the simulator is chosen, the user must then click the run button (displayed as a play icon) in the top left corner. If the destination is chosen to be a simulator, an iPhone simulator will then pop up on the screen and display the app. Wait for the app to load, do not navigate the iPhone screen yourself. The app will then open itself and can be used. The app is configured using the Build Settings, Schemes, and Build Phases settings of the CS50 app target in Xcode. 
